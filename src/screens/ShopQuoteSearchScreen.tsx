...
import { QUOTATIONS, QuotationCard } from "./MyShopScreen";

type Nav = NativeStackNavigationProp<RootStackParamList>;

// Quotation search — pushed from ใบเสนอราคา's app-bar search button. Same
// match rule as the old inline box: quote id, customer company, or item name.
export function ShopQuoteSearchScreen() {
    ...
    <View className="flex-1" style={{ backgroundColor: "#fafafa" }}>
      ...
      >
        <Text style={{ paddingHorizontal: 16, marginBottom: 10, fontSize: 12, color: "#737373" }}>
          {q ? `พบ ${results.length} ใบเสนอราคา` : `ใบเสนอราคาทั้งหมด ${QUOTATIONS.length} รายการ`}
        </Text>
        ...
        {results.length === 0 ? (
          <EmptyState
            ...
            subtitle="ลองค้นด้วยเลขใบเสนอ ชื่อลูกค้า หรือชื่อสินค้า"
          />
        ) : (
          <View style={{ paddingHorizontal: 16, gap: 14 }}>
            {results.map((d) => (
              <QuotationCard key={d.id} doc={d} onOpenDetail={() => nav.navigate("ShopDocDetail", { doc: d, kind: "qt" })} />
            ))}
          </View>
        )}
      </ScrollView>
