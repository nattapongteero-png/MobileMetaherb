...
import { CafeMemberDetailScreen } from "../screens/CafeMemberDetailScreen";
import { CafeMemberAddScreen } from "../screens/CafeMemberAddScreen";
import { CafePointsScreen } from "../screens/CafePointsScreen";
import { CafeStampCardScreen } from "../screens/CafeStampCardScreen";
import { CafeNotificationScreen } from "../screens/CafeNotificationScreen";
...
export type RootStackParamList = {
  ...
  CafeMemberDetail: { memberId: string };
  CafeMemberAdd: undefined;
  CafePoints: undefined;
  CafeStampCard: undefined;
  CafeNotification: undefined;
...
// Tab icons, per platform.
//
// iOS draws the bar itself and wants an SF Symbol name — that is what gives the
// tab bar its Liquid Glass treatment, so native must keep returning the
// `{ sfSymbol }` object and nothing else. The web has no symbol catalogue, so
// the same four glyphs ship as PNGs (assets/tabicons, regenerated by
// scripts/genicons.mjs) and render as a tinted <Image>. Without this the web
// bar has labels and no icons at all.
const TAB_ICON_PNG = {
  home: require("../../assets/tabicons/home.png"),
  box: require("../../assets/tabicons/box.png"),
  leaf: require("../../assets/tabicons/leaf.png"),
  person: require("../../assets/tabicons/person.png"),
} as const;

// The return is cast because react-native-bottom-tabs types `tabBarIcon` for
// the native bar only — an SF Symbol or an image source, never an element. On
// web the navigator is the shim in src/shims, which does render the element.
type TabIcon = NonNullable<React.ComponentProps<typeof Tab.Screen>["options"] extends
  infer O ? O extends { tabBarIcon?: infer F } ? F : never : never>;

function tabIcon(sfSymbol: AppleIcon["sfSymbol"], png: keyof typeof TAB_ICON_PNG): TabIcon {
  if (Platform.OS !== "web") return () => ({ sfSymbol });
  return ((({ color, size }: { color?: string; size?: number } = {}) => (
    <Image
      source={TAB_ICON_PNG[png]}
      style={{ width: size ?? 22, height: size ?? 22, tintColor: color }}
      resizeMode="contain"
    />
  )) as unknown) as TabIcon;
}
  ...
  return (
    ...
    >
      <Tab.Screen
        name="Home"
        component={HomeTab}
        options={{ title: "หน้าแรก", tabBarIcon: tabIcon("house.fill", "home") }}
      />
      <Tab.Screen
        name="TrialProducts"
        component={TrialTab}
        // iPad has room for the full name; phones keep the short label.
        options={{ title: isTablet() ? "ผลิตภัณฑ์ทดสอบ" : "ทดลอง", tabBarIcon: tabIcon("shippingbox.fill", "box") }}
      />
      <Tab.Screen
        name="HerbalMarket"
        component={HerbalTab}
        options={{ title: isTablet() ? "Herbal Market" : "Market", tabBarIcon: tabIcon("leaf.fill", "leaf") }}
      />
      <Tab.Screen
        name="Account"
        component={AccountTab}
        options={{ title: "ฉัน", tabBarIcon: tabIcon("person.fill", "person") }}
      />
    </Tab.Navigator>
  );
}
...
export function RootStack() {
  return (
    ...
    >
      ...
      <Stack.Screen name="CafeMemberDetail" component={CafeMemberDetailScreen} options={{ headerShown: false, animation: "slide_from_right" }} />
      <Stack.Screen name="CafeMemberAdd" component={CafeMemberAddScreen} options={{ headerShown: false, animation: "slide_from_right" }} />
      <Stack.Screen name="CafePoints" component={CafePointsScreen} options={{ headerShown: false, animation: "slide_from_right" }} />
      <Stack.Screen name="CafeStampCard" component={CafeStampCardScreen} options={{ headerShown: false, animation: "slide_from_right" }} />
      <Stack.Screen name="CafeNotification" component={CafeNotificationScreen} options={{ headerShown: false, animation: "slide_from_right" }} />
